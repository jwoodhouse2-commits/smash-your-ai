...
function layout({ title, description, body, activeLessonKey = null, hasEntitlement = false, path = '/course', ogImage = 'https://smashyourai.com/images/og-default.jpg', pageType = 'website', breadcrumbs = null, jsonLd = null }) {
  const canonicalUrl = `https://smashyourai.com${path}`;
  const fullTitle = `${title} · Smash Your AI Course`;

  // Build BreadcrumbList JSON-LD if breadcrumbs provided.
  // breadcrumbs = [{ name, path }] in display order.
  const breadcrumbJsonLd = breadcrumbs && breadcrumbs.length
    ? {
        ...
        itemListElement: breadcrumbs.map((b, i) => ({
          ...
          position: i + 1,
          name: b.name,
          item: `https://smashyourai.com${b.path}`,
        })),
      }
    : null;

  const jsonLdBlocks = [breadcrumbJsonLd, jsonLd]
    .filter(Boolean)
...
<meta property="og:site_name" content="Smash Your AI">
...
function renderTierPage({ course, tier, completedKeys = new Set(), hasEntitlement }) {
  ...
  const tierPath = `/course/${tier.slug}`;
  const breadcrumbs = [
    { name: 'Home', path: '/' },
    { name: 'Course', path: '/course' },
    { name: tier.title, path: tierPath },
  ];
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Course',
    name: `${tier.title} · Smash Your AI Course`,
    description: tier.blurb,
    url: `https://smashyourai.com${tierPath}`,
    provider: {
      '@type': 'Organization',
      name: 'Smash Your AI',
      sameAs: 'https://smashyourai.com',
    },
    courseCode: tier.slug,
    educationalLevel: tier.slug,
    hasCourseInstance: {
      '@type': 'CourseInstance',
      courseMode: 'online',
      courseWorkload: `PT${Math.round(tier.modules.reduce((s, m) => s + m.lessons.reduce((ss, l) => ss + (l.duration || 0), 0), 0))}M`,
    },
    ...
    offers: {
      ...
      url: 'https://smashyourai.com/course',
    },
  };
  return layout({ title: tier.title, description: tier.blurb, body, hasEntitlement, path: tierPath, breadcrumbs, jsonLd });
}
...
function renderLessonPage({ course, tier, mod, lesson, completedKeys = new Set(), hasEntitlement, locked }) {
  ...
  const lessonPath = `/course/${tier.slug}/${mod.slug}/${lesson.slug}`;
  const breadcrumbs = [
    { name: 'Home', path: '/' },
    { name: 'Course', path: '/course' },
    { name: tier.title, path: `/course/${tier.slug}` },
    { name: mod.title, path: `/course/${tier.slug}/${mod.slug}/${mod.lessons[0].slug}` },
  ...
  const jsonLd = {
    ...
    name: lesson.title,
    description: lesson.summary || tier.title,
    url: `https://smashyourai.com${lessonPath}`,
    learningResourceType: 'Lesson',
    educationalLevel: tier.slug,
    timeRequired: lesson.duration ? `PT${lesson.duration}M` : undefined,
    isPartOf: {
      '@type': 'Course',
      name: `${tier.title} · Smash Your AI Course`,
      url: `https://smashyourai.com/course/${tier.slug}`,
    },
    provider: {
      '@type': 'Organization',
      name: 'Smash Your AI',
    ...
    isAccessibleForFree: !!(mod.isFreeModule),
  };
  return layout({ title: lesson.title, description: lesson.summary || tier.title, body, activeLessonKey: lesson.key, hasEntitlement, path: lessonPath, pageType: 'article', breadcrumbs, jsonLd });
}
